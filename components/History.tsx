            ...
            >
              ...
              <div className="flex items-center gap-4">
                <div className="text-right">
                  <div className="text-xs text-stone-500">总额</div>
              ...
              <div className="border-t border-stone-200">
                <table className="min-w-full divide-y divide-stone-100 text-sm">
                   <thead className="bg-white">
                      <tr>
                        <th className="px-6 py-2 text-left text-xs font-medium text-stone-500 uppercase">规格</th>
                        <th className="px-6 py-2 text-left text-xs font-medium text-stone-500 uppercase">详情</th>
                        <th className="px-6 py-2 text-right text-xs font-medium text-stone-500 uppercase">价格</th>
                      </tr>
                   </thead>
                   <tbody className="divide-y divide-stone-100 bg-white">
                     {batch.items.map(item => (
                       <tr key={item.id} className="hover:bg-stone-50">
                         <td className="px-6 py-3 font-medium text-brand-900">{item.specName}</td>
                         <td className="px-6 py-3 text-stone-600">{item.details}</td>
                         <td className="px-6 py-3 text-right font-bold text-accent-600">¥{item.totalRetail.toLocaleString()}</td>
                       </tr>
                     ))}
                     <tr className="bg-stone-50">
                       <td colSpan={2} className="px-6 py-3 text-right font-medium text-stone-900">本次合计成本</td>
                       <td className="px-6 py-3 text-right text-stone-900">¥{batch.totalCost.toLocaleString()}</td>
                     </tr>
                   </tbody>
                </table>
              </div>
